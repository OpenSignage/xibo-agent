  ...
  execute: async ({ context }): Promise<z.infer<typeof successSchema> | z.infer<typeof errorSchema>> => {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(`getWidgetData: HTTP error occurred: ${response.status}`, {
      ...
      if (error instanceof z.ZodError) {
        logger.error("getWidgetData: Validation error", { error: error.issues });
        return { success: false, message: "Validation error occurred", errorData: error.issues };
      }
      const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
      logger.error("getWidgetData: An unexpected error occurred", { error: errorMessage });
