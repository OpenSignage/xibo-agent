 ...
 * @description Provides a tool to add a new row of data to a dataset in the Xibo CMS.
 */
import { z } from "zod";
...
export const addDataSetData = createTool({
  ...
  execute: async ({ context }) => {
    ...
    const url = new URL(`${config.cmsUrl}/api/dataset/${dataSetId}/data`);
    logger.info(`Attempting to add data to dataset ID: ${dataSetId}`);
    ...
    try {
      ...
      const message = `Successfully added data to dataset ID: ${dataSetId}.`;
      logger.info(message, { rowId: validationResult.data.id });
      return {
