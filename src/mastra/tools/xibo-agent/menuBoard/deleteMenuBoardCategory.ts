...
/**
 * @module
 * This module provides a tool to delete a menu board category from the Xibo CMS.
 */
...
export const deleteMenuBoardCategory = createTool({
  ...
  execute: async ({ context: input }): Promise<z.infer<typeof outputSchema>> => {
    try {
      ...
      const url = `${config.cmsUrl}/api/menuboard/${input.menuCategoryId}/category`;
      logger.debug(`deleteMenuBoardCategory: Requesting URL = ${url}`);

      // Make the API call to delete the menu board category
      ...
      if (!response.ok) {
        ...
        logger.error(`deleteMenuBoardCategory: HTTP error: ${response.status}`, { error: errorData });
      ...
      logger.error('deleteMenuBoardCategory: An unexpected error occurred', { error });
