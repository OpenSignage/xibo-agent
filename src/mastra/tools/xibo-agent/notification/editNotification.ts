...
/**
 * @module EditNotification
 * @description This module provides a tool for updating existing notifications
 * in the Xibo CMS. It handles the API request, data validation, and response
 * formatting for editing a notification.
 */
import { z } from "zod";
...
/**
 * Defines the output schema for the editNotification tool.
 * It represents a successful response with the updated notification data or a failure.
 */
...
/**
 * Tool for updating an existing notification in the Xibo CMS.
 * This tool sends a PUT request to modify a notification's details.
 */
export const editNotification = createTool({
  id: 'edit-notification',
  description: 'Update an existing notification in the Xibo CMS.',
  inputSchema: z.object({
    notificationId: z.number().describe('The ID of the notification to update.'),
  ...
  execute: async ({ context }) => {
    ...
    logger.info({ notificationId: context.notificationId }, "Attempting to update a notification.");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message = `Failed to update notification. API responded with status ${response.status}.`;
      ...
      const message = `Successfully updated notification: "${validationResult.data.subject}".`;
      ...
      const message = "An unexpected error occurred while updating the notification.";
